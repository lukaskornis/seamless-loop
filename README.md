...
Drop an audio **or video** file, get an **inaudible seamless loop**, then save it as a single loop or stretched to any length — seconds, minutes, or hours. Everything runs **locally in your browser**; your audio is never uploaded anywhere.
...
- **Drag & drop** any audio *or video* the browser can decode — WAV, MP3, OGG, FLAC, M4A, **MP4, MOV, WebM**… (the audio track is pulled straight out of video; the picture is ignored).
...
- **Constant-power crossfade loop** — folds the tail back over the head so the end → start transition is click-free, using a **square-root constant-power** curve that keeps loudness flat across the seam (no quiet dip), with an adjustable crossfade (default **400 ms**) and optional **zero-crossing snap**.
...
1. Open the [app](https://lukaskornis.github.io/seamless-loop/) and drop in an audio or video file.
...
The loop unit is built with a **square-root constant-power crossfade**: the last *f* milliseconds are blended over the first *f* milliseconds with gains `√t` and `√(1−t)`, whose squares always sum to 1 — so the combined power stays flat through the seam (no linear sag, no quiet patch) while the boundary lands on consecutive samples from the original. No click, no pop, no dip. Optional zero-crossing snap aligns the loop edges for even cleaner results.
